...
# ---------------------------------------------------------------------------
# __main__: self-tests and verified constants
# ---------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    # -------------------------------------------------------------------------
    print("=== Hand-verified constants ===")

    # 1.0: sign=0, exp=16383, sig=0
    ONE     = pack(0, 16383, 0)
    NEG_ONE = pack(1, 16383, 0)
    TWO     = pack(0, 16384, 0)
    HALF    = pack(0, 16382, 0)
    ONE25   = pack(0, 16383, 1 << 110)   # 1.25 = 1 + 0.25, 0.25 = 1/4 = 2^-2 → bit 110
    ZERO    = 0
    NZERO   = 1 << 127
    INF_P   = 0x7FFF << 112
    ...
    for name, bits, s_exp, e_exp, t_exp in [
        ...
        s, e, t = unpack(bits)
        check(f'unpack({name}) sign', s, s_exp)
        check(f'unpack({name}) exp',  e, e_exp)
        check(f'unpack({name}) sig',  t, t_exp)
        print(f"  {name}: 0x{bits:032X}  sign={s} exp=0x{e:04X} sig=0x{t:028X}")

    # Verify 1.25 bit pattern more carefully:
    # 1.25 = 1 + 1/4 = 1.01 in binary.
    # sig = 0.25 * 2^112 = 2^110
    assert ONE25 == pack(0, 16383, 1 << 110), "1.25 encoding mismatch"
    print(f"  1.25 sig = 2^110 = {1<<110} OK")

    # Smallest subnormal: sign=0, exp=0, sig=1
    # value = 2^(1-16383) * 1/2^112 = 2^(-16494)
    ms_frac = to_fraction(MIN_SUB)
    expected_ms = Fraction(1, 1 << 16494)
    check('min_subnormal value', ms_frac, expected_ms)
    print(f"  min_subnormal value = 2^(-16494): {ms_frac == expected_ms}")

    # to_halves of 1.0
    lo, hi = to_halves(ONE)
    # ONE = 0x3FFF0000000000000000000000000000
    # HI = 0x3FFF000000000000, LO = 0x0000000000000000
    check('to_halves(1.0) hi', hi, 0x3FFF000000000000)
    check('to_halves(1.0) lo', lo, 0)
    print(f"  to_halves(1.0) = (lo=0x{lo:016X}, hi=0x{hi:016X})")

    # 1.25 halves: exp=16383=0x3FFF, sig top 48 bits = (1<<110) >> 64 = 1<<46
    lo25, hi25 = to_halves(ONE25)
    # sig = 2^110; bits [111:64] of sig = 2^(110-64) = 2^46
    check('to_halves(1.25) hi', hi25, 0x3FFF000000000000 | (1 << 46))
    check('to_halves(1.25) lo', lo25, 0)
    print(f"  to_halves(1.25) = (lo=0x{lo25:016X}, hi=0x{hi25:016X})")

    # +inf halves: 0x7FFF000000000000 / 0
    lo_inf, hi_inf = to_halves(INF_P)
    check('to_halves(+inf) hi', hi_inf, 0x7FFF000000000000)
    check('to_halves(+inf) lo', lo_inf, 0)
    print(f"  to_halves(+inf) = (lo=0x{lo_inf:016X}, hi=0x{hi_inf:016X})")
    ...
    check('1+1=2', add(ONE, ONE), TWO)
    check('2-1=1', sub(TWO, ONE), ONE)
    check('1/2=0.5', div(ONE, TWO), HALF)
    check('0.5*2=1', mul(HALF, TWO), ONE)
    check('inf+(-inf)=nan', _is_nan(add(INF_P, INF_N)), True)
    check('1/0=+inf', div(ONE, ZERO), INF_P)
    check('-1/0=-inf', div(pack(1, 16383, 0), ZERO), INF_N)
    check('0/0=nan', _is_nan(div(ZERO, ZERO)), True)
    check('inf*0=nan', _is_nan(mul(INF_P, ZERO)), True)
    check('nan+1=nan', _is_nan(add(NAN_C, ONE)), True)

    # 1/3 + 2/3 = 1 (approximately — won't be exact due to rounding)
    TWO_THIRDS = div(TWO, add(ONE, TWO))
    ONE_THIRD  = div(ONE, add(ONE, TWO))
    # They should add to either 1.0 or the nearest f128 to 1.0
    sum_third = add(ONE_THIRD, TWO_THIRDS)
    frac_sum = to_fraction(sum_third)
    check('1/3+2/3 ~ 1 (within 1 ulp of 1)', abs(frac_sum - 1) <= Fraction(1, 1 << 112), True)

    # Overflow to infinity
    check('max_finite*2=+inf', div(_MAX_FINITE, HALF), INF_P)

    # Subnormal arithmetic
    check('2*min_sub', mul(TWO, MIN_SUB), pack(0, 0, 2))
    check('min_sub/2 = 0 or underflow', mul(MIN_SUB, HALF) in (ZERO, pack(0,0,0)), True)
    ...
    check('eq(1,1)', eq(ONE, ONE), True)
    check('eq(+0,-0)', eq(ZERO, NZERO), True)
    check('eq(nan,nan)', eq(NAN_C, NAN_C), False)
    check('ne(nan,1)', ne(NAN_C, ONE), True)
    check('lt(1,2)', lt(ONE, TWO), True)
    check('lt(2,1)', lt(TWO, ONE), False)
    check('lt(nan,1)', lt(NAN_C, ONE), False)
    check('ge(1,1)', ge(ONE, ONE), True)
    check('gt(inf,max)', gt(INF_P, _MAX_FINITE), True)
    check('lt(-inf,-1)', lt(INF_N, NEG_ONE), True)
    ...
    check('from_int(0)', from_int(0), ZERO)
    check('from_int(1)', from_int(1), ONE)
    check('from_int(2)', from_int(2), TWO)
    check('from_int(-1)', from_int(-1), NEG_ONE)
    check('to_int(1.0)', to_int(ONE), 1)
    check('to_int(-1.0)', to_int(NEG_ONE), -1)
    check('to_int(inf)', to_int(INF_P), 0)
    check('to_int(nan)', to_int(NAN_C), 0)
    # to_int(1.75) = 1 (truncate toward zero)
    ONE75 = parse('1.75')
    check('to_int(1.75)', to_int(ONE75), 1)
    check('to_int(-1.75)', to_int(parse('-1.75')), -1)
    # Large integer round-trip
    for n in [0, 1, -1, 2**52, 2**63, -(2**63), 10**30]:
        rt = to_int(from_int(n))
        check(f'int round-trip {n}', rt, n)
    ...
    check('from_f64(1.0)', from_f64(F64_ONE), ONE)
    check('from_f64(2.0)', from_f64(F64_TWO), TWO)
    check('from_f64(0.5)', from_f64(F64_HALF), HALF)
    check('from_f64(+inf)', from_f64(F64_INF), INF_P)
    check('from_f64(-inf)', from_f64(F64_NINF), INF_N)
    check('to_f64(1.0)', to_f64(ONE), F64_ONE)
    check('to_f64(2.0)', to_f64(TWO), F64_TWO)
    check('to_f64(0.5)', to_f64(HALF), F64_HALF)
    check('to_f64(+inf)', to_f64(INF_P), F64_INF)
    check('to_f64(-inf)', to_f64(INF_N), F64_NINF)
    check('to_f64(nan) is nan', (to_f64(NAN_C) >> 52) & 0x7FF, 0x7FF)
    # Round-trip representable f64 values
    for bits64, name in [(F64_ONE, '1.0'), (F64_TWO, '2.0'), (F64_HALF, '0.5'),
                         (0x4008000000000000, '3.0'), (0x400921FB54442D18, 'pi_f64')]:
        rt = to_f64(from_f64(bits64))
        check(f'f64 round-trip {name}', rt, bits64)
    ...
    check("parse('1.0')",  parse('1.0'),  ONE)
    check("parse('2.0')",  parse('2.0'),  TWO)
    check("parse('0.5')",  parse('0.5'),  HALF)
    check("parse('1.25')", parse('1.25'), ONE25)
    check("parse('-1.0')", parse('-1.0'), NEG_ONE)
    check("parse('inf')",  parse('inf'),  INF_P)
    check("parse('-inf')", parse('-inf'), INF_N)
    check("parse('nan')",  _is_nan(parse('nan')), True)
    check("parse('0.0')",  parse('0.0'),  ZERO)
    check("parse('1e0')",  parse('1e0'),  ONE)
    check("parse('2e0')",  parse('2e0'),  TWO)
    # parse('1.5'): 1.5 = 1 + 0.5 → sig = 2^111
    check("parse('1.5')",  parse('1.5'), pack(0, 16383, 1 << 111))
    ...
    check("format_f(1.0)",   format_f(ONE),    '1.000000')
    check("format_f(2.0)",   format_f(TWO),    '2.000000')
    check("format_f(0.5)",   format_f(HALF),   '0.500000')
    check("format_f(1.25)",  format_f(ONE25),  '1.250000')
    check("format_f(-1.0)",  format_f(NEG_ONE),'-1.000000')
    check("format_f(+0)",    format_f(ZERO),   '0.000000')
    check("format_f(-0)",    format_f(NZERO),  '-0.000000')
    check("format_f(+inf)",  format_f(INF_P),  'inf')
    check("format_f(-inf)",  format_f(INF_N),  '-inf')
    check("format_f(nan)",   format_f(NAN_C),  'nan')
    # 1/3 → 0.333333
    check("format_f(1/3)",   format_f(div(ONE, add(ONE,TWO))), '0.333333')
    # 2/3 → 0.666667
    check("format_f(2/3)",   format_f(div(TWO, add(ONE,TWO))), '0.666667')
    # 100.0
    check("format_f(100.0)", format_f(from_int(100)), '100.000000')
    # 0.1
    check("format_f(0.1)",   format_f(parse('0.1')), '0.100000')
    # 1e-7 → rounds to 0.000000
    check("format_f(1e-7)",  format_f(parse('0.0000001')), '0.000000')
    # large number
    check("format_f(1e6)",   format_f(parse('1000000.0')), '1000000.000000')
    ...
    check("format_g(1.0)",      format_g(ONE),    '1')
    check("format_g(2.0)",      format_g(TWO),    '2')
    check("format_g(0.5)",      format_g(HALF),   '0.5')
    check("format_g(1.25)",     format_g(ONE25),  '1.25')
    check("format_g(-1.0)",     format_g(NEG_ONE),'-1')
    check("format_g(+0)",       format_g(ZERO),   '0')
    check("format_g(-0)",       format_g(NZERO),  '-0')
    check("format_g(+inf)",     format_g(INF_P),  'inf')
    check("format_g(-inf)",     format_g(INF_N),  '-inf')
    check("format_g(nan)",      format_g(NAN_C),  'nan')
    check("format_g(0.0001)",   format_g(parse('0.0001')),   '0.0001')
    check("format_g(0.00001)",  format_g(parse('0.00001')),  '1e-05')
    check("format_g(100000)",   format_g(from_int(100000)),  '100000')
    check("format_g(1000000)",  format_g(from_int(1000000)), '1e+06')
    check("format_g(123456)",   format_g(from_int(123456)),  '123456')
    check("format_g(1234567)",  format_g(from_int(1234567)), '1.23457e+06')
    check("format_g(3.14159)",  format_g(parse('3.14159')),  '3.14159')
    check("format_g(3.141592653)", format_g(parse('3.141592653')), '3.14159')
    check("format_g(1e-5)",     format_g(parse('1e-5')),     '1e-05')
    check("format_g(1.5e10)",   format_g(parse('1.5e10')),   '1.5e+10')
    check("format_g(0.1)",      format_g(parse('0.1')),      '0.1')
    check("format_g(0.0)",      format_g(ZERO),              '0')

    # -------------------------------------------------------------------------
    # vectors() self-consistency
    # -------------------------------------------------------------------------
    print("\n=== vectors() self-consistency ===")
    ...
    ops = {
        ...
        'from_int': from_int, 'to_int': to_int,
        'from_f64': from_f64, 'to_f64': to_f64,
        'parse': parse, 'format_f': format_f, 'format_g': format_g,
        'unpack': unpack,
    }
    ...
    for vec in vecs:
        ...
        if op == 'from_halves':
            lo, hi = inp[0]
            got = from_halves(lo, hi)
        elif op == 'parse_format_f':
            # Check that parse then format_f gives back the same string
            got = format_f(parse(inp[0]))
        elif op == 'from_f64_to_f64':
            bits64 = inp[0][0]
            got = to_f64(from_f64(bits64))
        elif op in ops:
            fn = ops[op]
            if op in ('format_f', 'format_g', 'from_int', 'to_int', 'from_f64', 'to_f64',
                      'parse', 'unpack'):
                got = fn(inp[0])
            elif op in ('eq','ne','lt','le','gt','ge','add','sub','mul','div'):
                got = fn(inp[0], inp[1])
            else:
                got = fn(*inp)
        else:
        ...
        # For NaN results, compare as both NaN.
        if isinstance(exp, int) and isinstance(got, int) and _is_nan(exp) and _is_nan(got):
            vec_pass += 1
            passed += 1
        elif got == exp:
            vec_pass += 1
            ...
            vec_fail += 1
            failed += 1
            errors.append(f"FAIL vec [{op}] {note}\n  got      {got!r}\n  expected {exp!r}")
    ...
    # (a+b)-b ~ a for some values
    for a_str, b_str in [('1.0', '0.5'), ('3.14159', '2.71828'), ('100.0', '0.001')]:
        a = parse(a_str)
        b = parse(b_str)
        result = sub(add(a, b), b)
        # Should be exactly a for these well-spaced values
        fa = to_fraction(a)
        fr = to_fraction(result)
        rel = abs(fr - fa)
        # Allow 1 ulp
        check(f'({a_str}+{b_str})-{b_str} ~ {a_str}', rel <= abs(fa) * Fraction(1, 1<<112), True)

    # parse(format_f(x)) ~ x for normals
    for s in ['1.0', '0.5', '1.25', '100.0', '3.14159']:
        x = parse(s)
        s2 = format_f(x)
        x2 = parse(s2)
        # They should agree to at least 6 decimal places.
        fx = to_fraction(x)
        fx2 = to_fraction(x2)
        if fx != 0:
            rel_err = abs(fx2 - fx) / abs(fx)
            check(f'parse(format_f({s})) close', rel_err < Fraction(1, 10**5), True)

    # from_int / to_int round-trip for integers in f128 range
    for n in [0, 1, -1, 42, -100, 2**50, -(2**50), 2**113, -(2**113)]:
        check(f'to_int(from_int({n}))', to_int(from_int(n)), n)

    # -------------------------------------------------------------------------
    # Summary
    # -------------------------------------------------------------------------
    print(f"\n{'='*50}")
    print(f"Vectors generated: {len(vecs)}")
    categories = {}
    for vec in vecs:
        categories[vec['op']] = categories.get(vec['op'], 0) + 1
    for op, cnt in sorted(categories.items()):
        print(f"  {op:20s}: {cnt}")
    print(f"\nVector tests:  {vec_pass} passed, {vec_fail} failed")
    print(f"Unit tests:    {passed - vec_pass} passed, {failed - vec_fail} failed")
    print(f"{'='*50}")
