...
- **memory_field_receiver_method_move.fire** - Regression: an owned parameter/local moved via a method call whose *receiver* is a field access (`this.field.method(x)`) -- both a non-generic field (`this.sink.take(s)`) and a generic-typed one (`this.items.push(s)`, a `Vec<string>` field) -- is now correctly recognized as moved, so it isn't also auto-dropped at scope exit (previously an internal FIR verifier crash, FIRV-O2). Two root causes: `semantic_analyzer.py`'s `METHOD_CALL` handling and `preprocessor.py`'s drop-insertion pass each only resolved a receiver's type for a bare-identifier receiver; and once fixed, the generic-field case needed the same "strip generic type arguments before the `method_sigs` lookup" normalization `ast_to_fir.py` already applied elsewhere. Discovered while building `@firescript/std.text.StringBuilder`.
...
- **result_isok_iserr.fire** - `Result<T,E>` basic usage: `Ok`/`Err` construction via the direct constructor (`Result<T,E>(value, error)` with the unused side `null` — no static `Ok()`/`Err()` factories, since static methods on generic classes don't work yet, see `tests/sources/known_issues/generic_class_static_method_crash.fire`) and `isOk()`/`isErr()` correctness for Owned `T`/`E`
...
### `std/text/`
- **stringbuilder_basic.fire** - `StringBuilder`: `append()` accumulates fragments, `length()` tracks total character count, `build()` joins and drains them (`length()` reads 0 afterward). No zero-arg constructor -- `StringBuilder("")` starts empty; see `known_issues/zero_arg_constructor_bare_call_error.fire`.
- **split_basic.fire** - `split(s, delim)`: splits on every occurrence including leading/trailing/adjacent empty fields, no-match (whole string as a single element), and a multi-character delimiter. Returns `Vec<string>`, drained here with `pop()` (LIFO) rather than `get()`/`enumerate()`, which aren't safe for an Owned element type.
...
- **generic_class_static_method_crash.fire** - A `static fn` on a generic class cannot be called in any form: explicit type arguments (`Box<int32>.make(5)`) are a parse error (the parser only expects a constructor call immediately after generic type arguments), and inferred type arguments (`Box.make(5)`) crash AST->FIR conversion with `FIRV-L1` ('Box' has no dominating parameter, DeclareLocal, or global constant -- the bare class name is lowered as a variable load instead of a static-call target). Discovered while designing `Result<T,E>`'s `Ok`/`Err` constructors (`std/types/result_isok_iserr.fire`), which use direct construction instead as a result.
- **zero_arg_constructor_bare_call_error.fire** - A same-file class's bare `Foo(args)` constructor call only resolves against its real declared constructor when the param list happens to match the class's field list (count and types); `parser/type_system.py`'s "func_name in self.user_types" branch otherwise re-validates the call against field order as if there were no explicit constructor, producing a wrong (sometimes wildly misleading) arg-count or arg-type error. `new Foo(args)` avoids this for a same-file class. Discovered while designing `@firescript/std.text.StringBuilder`.
- **zero_arg_constructor_field_access_after_bare_call.fire** - Related, separate symptom: constructing an instance of a class *imported from another module* via bare `ClassName(args)` (any arg count) type-checks fine, but the resulting variable's type isn't correctly registered for a later raw field access on it ("Field access on non-class type"); calling a *method* on the same instance works fine. Uses a helper module (`widget_helper.fire`) to exercise the cross-module path. Discovered while designing `@firescript/std.text.StringBuilder`; worked around by never exposing raw field access from outside the class.
