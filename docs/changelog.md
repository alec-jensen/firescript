...
- Added `Result<T,E>`/`CopyableResult<T,E>` to `@firescript/std.types`, representing a success (`T`) or failure (`E`) value with `isOk()`/`isErr()`.
- Added `Some(value)`/`None()` static factories to `Option<T>`/`CopyableOption<T>`, and `Ok(value)`/`Err(error)` static factories to `Result<T,E>`/`CopyableResult<T,E>`. `None()`/`Err()` (and any factory whose own arguments don't mention every type parameter) require the explicit type-argument form (`Option<int32>.None()`, `Result<int32, string>.Err("...")`) since inference can't recover a type parameter absent from the call's arguments. Added `unwrapOr(default)` to all four classes, returning the held value if present/`Ok` else `default`; on the Owned classes (`Option`, `Result`) this drains the container (the value is extracted, not just read), matching the safety argument `Vec<T>.pop()` already relies on. See `docs/reference/std/types.md`.
- Added `.indexOf()`, `.substring()`, `.startsWith()`, `.endsWith()`, `.trim()`, and `.replace()` methods on `string`, alongside the existing `.length()`/`.upper()`/`.lower()`. No import needed. `.replace()` replaces every occurrence (not just the first); an empty `old` argument is a no-op. `.trim()` strips leading/trailing space, tab, `\n`, and `\r`.
- Added `@firescript/std.text` with `StringBuilder` (efficient accumulation of many string fragments via `append`/`length`/`build`) and `split` (dividing a string on a delimiter, returning `Vec<string>`) — see `docs/reference/std/text.md`.
...
- An owned parameter or local moved via a method call whose *receiver* is a field access (e.g. `this.parts.push(s)`) — rather than a bare identifier (`parts.push(s)`) — is now correctly recognized as moved, so it's no longer also auto-dropped at scope exit (previously an internal FIR verifier crash, `FIRV-O2`, "use of 's', which is moved"). Two compounding gaps: `semantic_analyzer.py`'s method-call handling and `preprocessor.py`'s drop-insertion pass each only resolved a receiver's type for a bare-identifier receiver, leaving a field-access receiver's type unresolved and its consumed arguments never marked moved; and once that was fixed, a *generic*-typed field (e.g. `Vec<string>`) needed the same "strip generic type arguments before the signature lookup" normalization already applied elsewhere in the compiler, since the field's resolved type string carries the full instantiation (`Vec<string>`) but signatures are keyed by the bare class name (`Vec`).
- A same-file class's bare `ClassName(args)` constructor call now always resolves against the class's real declared constructor (matching `new ClassName(args)`) instead of sometimes being misresolved as an *implicit* field-order constructor whenever the declared constructor's parameters don't exactly match the class's fields (in count or type) — previously producing a wrong, sometimes wildly misleading arg-count or arg-type error (e.g. a zero-parameter constructor on a one-field class reporting "expected 1 args, got 0").
- Reading a field on an instance constructed via bare `ClassName(args)`, where `ClassName` is a class imported from another module, no longer fails with a spurious "Field access on non-class type" error — the class's type wasn't yet registered at the point this file's own (pre-merge) parse ran, a gap previously worked around only for composite generic types (`Tuple<int32, string>`), not a plain imported class name. Calling a *method* on the same instance was unaffected by this bug.
- A `static fn` on a generic class can now be called, with either inferred (`Box.make(5)`) or explicit (`Box<int32>.make(5)`) type arguments — previously the inferred form crashed AST->FIR conversion (`FIRV-L1`, the bare class name lowering as a variable load instead of a static-call target) and the explicit form was a parse error ("Expected '(' after generic class type arguments"). A generic class's bare template name is deliberately absent from the parser's concrete-class registries (tracked as a separate template entry instead), which the `Type.method(...)`/`Identifier<TypeArgs>.method(...)` parsing special cases, `TYPE_METHOD_CALL` type-checking, and FIR monomorphization all needed a fallback for; the inferred form additionally needed its type arguments inferred from the call's own arguments, since there's no receiver instance to read them from.
- Assigning `null` to a nullable-scalar field or local from within a class method (e.g. `this.value = null;` where the field's type is a generic `T` instantiated as a scalar like `int32`) no longer crashes FIR->FLIR verification with a store type mismatch — the assignment paths never propagated the target's expected type before converting the `null` right-hand side, so it lowered to a pointer-shaped null instead of the scalar's zero value, and separately the FLIR verifier's store check was the one place not using the codebase's established pointer-leniency comparison already applied to slot/call/binop checks.
- A generic class referencing itself inside its own body — a static factory returning `Box<T>` from within `class Box<T?>`, or one method calling another on `this` — no longer fails to parse or type-check when the file has no `import` statements at all (as in `@firescript/std.types`'s own source). The enclosing class isn't registered in the parser's normal class registries until its whole body finishes parsing, and a pre-existing fallback for this only activated in files that happened to have an unrelated import; the fix tracks the currently-parsing/type-checking generic class name directly, in both the parser's parse-time and separate type-check-time traversals.
- A bare (no explicit type arguments), type-qualified static method call on a class imported from another module (e.g. `Counter.fromDouble(5)`) no longer crashes AST->FIR conversion (`FIRV-L1`, "no dominating parameter, DeclareLocal, or global constant") — there was previously no deferred-import parsing fallback for the bare form at all (only the `Identifier<TypeArgs>.method(...)` form had one), so it silently fell through to being parsed as an ordinary instance method call on a plain identifier. This affected static calls on any imported class, generic or not.
