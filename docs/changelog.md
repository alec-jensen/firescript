...
- Added `Result<T,E>`/`CopyableResult<T,E>` to `@firescript/std.types`, representing a success (`T`) or failure (`E`) value with `isOk()`/`isErr()`. Constructed directly (`Result<T,E>(value, error)`, passing `null` for the unused side) rather than via `Ok`/`Err` static factories — see `docs/reference/std/types.md` for why.
